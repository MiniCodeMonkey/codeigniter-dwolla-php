...
class Dwolla {
    ...
    public function __construct($userConfig = array())
    {
        ...
        $CI =& get_instance();
        $CI->config->load('dwolla');

        $defaultConfig = array(
            'apiKey' => $CI->config->item('dwolla_api_key'),
            'apiSecret' => $CI->config->item('dwolla_api_secret'),
            'redirectUri' => $CI->config->item('dwolla_redirect_uri'),
            'permissions' => $CI->config->item('dwolla_permissions'),
    ...
    protected function _curl($url, $method = 'GET', $params = array())
    {
        ...
        curl_setopt($ch, CURLOPT_CAINFO, $ca . '/dwolla_cacert.pem'); // Set the location of the CA-bundle
